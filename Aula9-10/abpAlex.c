/*******************************************************************************
 ...
 Nome :   Alexandre Santos                                            NMEC : 80106
...
{ /* função recursiva - insira o seu codigo */ 
	...
	if(( Node -> PtLeft ) == NULL) // se sub árvore esquerda é nula
	{
		if( ( Node -> PtRight ) != NULL) // se sub árvore direita é diferente de NULL
		{
			Node = Node -> PtRight; //destroi ligaçao ao nó atual e liga à sub arvore direita
			(*proot) = Node;
		}else
		{
			*proot = NULL;		//valor do ponteiro = NULL uma vez que já não deve referenciar nenhum nó
		}
	}else
	{
		ABPDeleteMin(&(Node->PtLeft));	//continua a procura na sub árvore esquerda
...
{ /* função repetitiva - insira o seu codigo */  
	...
	PtABPNode Node = (*proot);
	PtABPNode prev = NULL;
	if(Node->PtRight == NULL){ // a raiz é o maior
		(*proot) = Node -> PtLeft;
		free(Node);
		return;
	}
	while(Node -> PtRight != NULL){ //enquanto existir sub arvore direita
		prev = Node;		
		Node = Node -> PtRight;
	}
	
	prev -> PtRight = Node -> PtLeft;
	free(Node);
...
{ /* função repetitiva - insira o seu codigo */ 
	...
	Error = OK;
	int min = ABPMaxRec(proot) -> Elem;
	
	PtABPNode Ret,Node;
	Node = proot;
	if(Node->Elem < pvalue){
		min = abs(pvalue - Node->Elem);
	}
	Ret = NULL;
	...
	while(Node != NULL){
		if(pvalue == Node->Elem){
			...
			if(pvalue < Node->Elem){		//se Node -> Elem maior que pvalue, Node passa a apontar para a sub arvore esquerda
				Node = Node -> PtLeft;
			}else{
				if(abs(pvalue-Node->Elem) < min){	//testar minimo
					Ret = Node;
				}
				if(Node->PtRight != NULL){	//se elemento da sub arvore direita for diferente de NULL, node passa a apontar para sub arvore direita, pois nao é garantido que nao exista
					Node = Node -> PtRight;										//nó com valor superior a Ret->Elem e inferior a pvalue
				}else{
					break;
				}
			}
	...
	if(Ret == NULL){		//caso não haja nenhum elemento menor que pvalue retorna NULL
		Error = NO_ELEM;
		return NULL;
	}
...
{ /* função recursiva - insira o seu codigo */ 
	...
	if(elem >= pvalue){
		...
		min= abs(pvalue-elem);
		if(Node->PtRight != NULL){				//testa elemento da sub arvore direita e passa faz chamada recursiva para a mesma se nela houver um valor maior que Node LEem
			comp1=(Node->PtRight)->Elem;	
			if(abs(pvalue-comp1) < min){			
				return ABPCeilValue (Node->PtRight,pvalue);
			}
		}
		if(Node->PtLeft != NULL){
			comp2=(Node->PtLeft)->Elem;		
			if(abs(pvalue-comp2) < min){
				if((Node->PtLeft)->PtLeft != NULL)				
					return ABPCeilValue (Node->PtLeft,pvalue);
			}
	...
	if(proot == NULL){
		...
		if(proot-> PtLeft != NULL){ 			
			if(proot-> PtRight != NULL){				
				count+= ABPRank(proot->PtLeft,pvalue) +  ABPRank(proot->PtRight,pvalue);
			}else{ 
				count+= ABPRank(proot->PtLeft,pvalue);
			}
		}else{
			if(proot -> PtRight != NULL){
				count+=  ABPRank(proot->PtRight,pvalue);	
			}	
		}
	}
...
{ /* função recursiva - insira o seu codigo */ 
	...
	if(phigh < plow){
		...
		Error = OK;
		int* elem = &proot->Elem;
		if(proot-> PtLeft != NULL){ 				//travessia da arvore em ordem, o que garante elementos ordenados
			ABPElements (proot->PtLeft,pqueue,plow,phigh);	
			if(*elem >= plow && *elem <=phigh){		
				QueueEnqueue(pqueue,elem);		//se elemento de proot dentro do intervalo [plow;phigh] inserir na fila
			}	
			if(proot-> PtRight != NULL){				
				ABPElements (proot->PtRight,pqueue,plow,phigh);
			}
		}else{	
			if(*elem >= plow && *elem <=phigh){		//se elemento de proot dentro do intervalo [plow;phigh] inserir na fila
...
int ABPIsEvenOdd (PtABPNode proot)			//uso parcial de função void ABPInOrderRep (PtABPNode proot) para organizar repetitivamente, em ordem, elementos na stack
{ /* função repetitiva - insira o seu codigo */ 
	if(proot == NULL){
		Error = ABP_EMPTY;		
		return 0;
	...
	int validateO=1;
	int count=0;
	int min= ABPMinRec(proot) -> Elem;
	PtStack ps = StackCreate(sizeof(int)); 
	StackPush(ps,&Node);
	while (!StackIsEmpty (ps))
	{
		...
		{
			Node = Node->PtLeft;
			StackPush (ps, &Node);	/* subarvore esquerda - left subtree */
		}
		else	/* é um no externo */
		{
			StackPop (ps, &Node);	/* retirar e descartar o no externo - retrieve and ignore the external node */
			if (!StackIsEmpty (ps))
			{
				StackPop (ps, &Node);	/* recuperar o no anterior - retrieve and preview node */
				if((min)%2 ==0){					
					if(count%2==0 && (Node->Elem)%2 !=0){
						validateE = 0;
						break;
					}
					if(count%2!=0 && (Node->Elem)%2 ==0){						
