/*
 * João Manuel André Coelho  Nº 80335
 * Tiago Jorge Gomes Almeida Nº 76404
 */
...
{ 
	int nvertices= pdig->NVertexes;
	if (pdig == NULL) return NO_DIGRAPH;
	if(pdig->NVertexes == 0) return DIGRAPH_EMPTY;
	PtBiNode aux = OutPosition(pdig->Head,pv);
	if (aux == NULL) return NO_VERTEX;	
	else{
		unsigned int Out = ((PtVertex)aux->PtElem)-> OutDeg;
		*pcent = (double)Out/(double)(nvertices-1);	
	}
...
{ 
	if (pdig == NULL) return NO_DIGRAPH;
	if(pdig->NVertexes == 0) return DIGRAPH_EMPTY;
	if(pv == NULL) return NULL_PTR ;
	if(pmax == NULL) return NULL_PTR;
	PtBiNode aux = pdig->Head;
	double pcent;
	double paux=0;
	unsigned int pvaux;
	while( aux != NULL){
		VertexOutDegreeCentrality(pdig,aux->Number,&pcent);
		if( pcent > paux){ 
			paux= pcent;
			pvaux = aux->Number;
		}
...
{ 
	...
	PtBiNode Vertice = OutPosition(pdig->Head, pv);
	
	if(pcent == NULL) return NULL_PTR;
	if(pdig == NULL) return NO_DIGRAPH;
	if(pdig->NVertexes == 0) return DIGRAPH_EMPTY;
	if(Vertice == NULL) return NO_VERTEX;
	
	PtVertex Vertex = (PtVertex) Vertice -> PtElem;
	
	if(Vertex->OutDeg == 0) return NO_EDGE;
	
	int sum = 0;
	Vertice = Vertice->PtAdj;
	
	for(;Vertice!=NULL;Vertice = Vertice->PtNext)
	{
		Vertex = (PtVertex) Vertice->PtAdj -> PtElem;
		sum += (Vertex->OutDeg);
	}
	Vertice = OutPosition(pdig->Head, pv);
	Vertex = (PtVertex) Vertice -> PtElem;
	
	*pcent = (double)sum / (double)Vertex->OutDeg;
...
{ 
	...
	PtBiNode Vert = pdig->Head;
	PtVertex Vertex;
	
	if((*pqueue = QueueCreate(sizeof(int))) == NULL) return NO_MEM;
	
	for(;Vert!=NULL;Vert = Vert->PtNext){
		Vertex = (PtVertex) Vert -> PtElem;
		if(Vertex->InDeg == 0 && Vertex->OutDeg == 0)
			if(QueueEnqueue(*pqueue,&Vert->Number) !=0)
				return NO_MEM;
	}
...
{ 
	...
	while(aux != NULL){
		PtBiNode adj = aux -> PtAdj;
		while(adj != NULL){
			if(adj->Number == pv){
				int err = QueueEnqueue(q, &(aux->Number));
				if(err == NO_MEM || err == NULL_PTR) return err;
			}
			adj = adj->PtNext;
		}
		aux = aux->PtNext;
...
{ 
	...
	int num = pdig->NVertexes;
	int verts[num][num];
	int i,a,j;
	for(i=1;i<=num;i++)
	{
		verts[i-1][i-1] = 1;
		aux = OutPosition(pdig->Head, i)->PtAdj;
		while(aux != NULL)
		{
			verts[i-1][(aux->Number)-1] = 1;
			aux = aux ->PtNext;	
		}
	}
	...
	for(a=1;a<=num;a++)
	{
		...
		{	
			if(verts[a-1][j-1]!=1)
			{
				int err = QueueEnqueue(q, &a);
				if(err == NO_MEM || err == NULL_PTR) return err;
				err = QueueEnqueue(q, &j);
				if(err == NO_MEM || err == NULL_PTR) return err;
			}
		}
	}
